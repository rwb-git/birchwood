...
//}
                  ...
                  // 12-12-2023 this date33 looks wrong, but it's only to check leap year, i think... but on jan 1 it fucks up by using day = 365 which is for a leap year, so it became jan 1 of 2024 which
                  // actually is a leap year; so subtract 1 here, same as below. the -2 below still might be fucked up, I don't know.


                  $dumrod = $day-1;
                  fwrite($flog,"create using day $dumrod  year $year ");
                  ...
                  //echo 'date33 line 238 ',$date33->format('d');          
                  $dumrod = $date33->format('d');
                  $dumrodyy = $date33->format('Y');
                  $dumrodmm = $date33->format('F');
                  $dumrodm2 = $date33->format('m');
                  fwrite($flog,"initial date33  $dumrod $dumrodyy month $dumrodmm  $dumrodm2  <br>");
...
// 1-2-2024 looks like php fixed the bug i used to compensate for and now my code does not work. but it does seem to work (see server_info.php) if i eliminate all the leap year code
...
/*
                  ...
                  fwrite($flog,"isleapyer says this $isLeapYear  ");



                  // php date time doy is 0..364 (or 365 leap year)
                  // calendar online shows doy 1..365 (or 366) with feb 29 = 60, so feb 29 in php format is day 59
...
// this looks wrong. feb 29 is day 59
                  ...
                  $dumrod = $date33->format('d');
                  fwrite($flog,"leap year day == 60 date33  $dumrod    ");
                  ...
                  $dumrod = $date33->format('d');
                  fwrite($flog,"leap year day is not 60 date33  $dumrod     ");
                        }
    ...
    //              $dumrod = $date33->format('z');
      //            fwrite($flog,"not leap year date33 or doy is less than feb 29 (60)  $dumrod    ");
                     //$date33 = DateTime::createFromFormat('z Y', ($day - 1) . ' ' . $year);  // java day 1..366  php day 0..355 so subtract 1
  //                }

                  $dumrod = $date33->format('d');
                  fwrite($flog,"final date33  $dumrod\n\n");
