   ...
   foreach ($data as &$number){ // lots of my code has & on the second arg, but that's only needed to alter the array itself which typically i don't want to do
      ...
      if ($cnt == 0){   
         ...
         if (($cnt % $two) == 0){    // even cnt = delta
            ...
            $new_num_recs++;  // 4-26-2021 when i converted this file for pulses, which do not have states, i screwed up by moving this line to the first part of this "if" block, so that for pulses it failed to count the first one.
                              // but for this file, the count is correct since this bit of code is executed for every entry including the first, so i think this file is ok as is.
...
//   $fgname = "new_test_block.txt";
   ...
   for ($i=0; $i<$match; $i++){
      ...
      if ($adjusted >= $mil24){
         ...
         if($adjusted < $first_new_millen){ // just keep old data that comes before new data. a weird error was happening
            ...
            fwrite($flog,"save rec from old data: $adjusted $old_states[$i]\n");
         }
      }
   ...
   for ($i=0;$i<$new_num_recs;$i++){
      ...
      if ($adjusted >= $mil24){
         ...
         fwrite($flog,"save rec from url data: $adjusted $new_states[$i]\n");
      }
