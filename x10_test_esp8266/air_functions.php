...
function draw_horizontal_grid(){  // $lines is the total including the top edge and bottom edge. $label_delt is the value on the next to the bottom line and the increment for all lines above it; for example, 200 will give you 200 400 600...
   ...
   for ($x = 0; $x <= $lines; $x++){
      ...
      if ($x <= ($lines )){  // example lines = 7: the last label will be x < 6 == x = 5

         // air will start with 10 at the bottom, with the three lines labeled 20 30 40


         $s = sprintf("%d",($the_number));   // example lines = 7: the next to the top line is the first one labeled: (7-1) x 10 - (0 x 10) = 60. the next to the bottom line will be labeled (7-1)x10 - (5x10) = 60 - 50 = 10
...
//   $previous_state = 0; // 12-25-2022 float and flow were on for 24 hours so it showed a blank. save the most recent state. if it's 0 and there is no data in 24 hours then
                        // force it on the whole time. actually just force this state and it will work.
