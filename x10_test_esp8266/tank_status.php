...
function draw_red_or_black($x1, $y1, $x2, $y2){
   ...
   if ($red_pix[intval(($x1 + $x2) / 2) - intval($plot_x1)] == 1){     // check the average of x1 and x2 instead of just x1. if the float is on in the midpoint of a segment that might work better than the first point, because
...
$fact_44 = 1.99; // this was 2 but top of plot was clipped, added 8-13-2022
...
if ($good_cnt >2){
   ...
   $x2 = $plot_x1; // added 8-13-2022 while doing air plot
   
      
   $y1 = intval($tank_y1 + 200 - ($fact_44 * $level[0]));
   $y2 = $y1; //$tank_y1 + 200 - (2 * $level[0]);
   ...
   for ($i = 1; $i < ($good_cnt); $i++){    //------------------------------- plot tank level graph ------------------------------------------
      ...
      if ($level[$i-1] == 100){
         ...
         $y2 = intval($tank_y1 + 200 - ($fact_44 * $level[$i]));
...
if ($data_is_old == 1){
   ...
   imagettftext($my_img->img, 32, 0, $big_num_x   , ($tank_y1 + $tank_y2)/2 - 20,  $red, $lib_font_reg, $s);   // img, size, angle, x, y, color, font, text

} else {

   $s = sprintf("%d",$val);

   imagettftext($my_img->img, 32, 0, $big_num_x , ($tank_y1 + $tank_y2)/2 - 20,  $text_colour, $lib_font_reg, $s);   // img, size, angle, x, y, color, font, text
}




// 8-31-2022 add air to tank plot
...
$old_offset = 0; //-(200/4); //10 * $fact73;  // offset is applied after scaling, so it's 10 * fact
//$old_offset = -(300/4); //10 * $fact73;  // offset is applied after scaling, so it's 10 * fact

$fact73 = 1.99; //(200-$old_offset) / 50;   // this was 5 for 300 tall and 0..60, since 300/60 = 5. new range is 10-50 so 300 / 40 = 7.5, with an offset of 10 x 7.5
...
if ($good_cnt >2){
   ...
   $y1 = intval($tank_y1 + 200 - $old_offset -($fact73 * $level[0] )); // fact73 was 2 for old tank 200 tall, 3.33 for air 200 tall, 5 for air 300 tall
   ...
   if ($x1 > $plot_x1){    // quite often the first data point will not be exactly at the left margin, so draw a line to it
      ...
      //draw_red_or_black($plot_x1, $y1, $x1, $y1);
      imageline( $my_img->img, $x1, $y1, $x2, $y2, $blue );
   }
   ...
   for ($i = 1; $i < ($good_cnt); $i++){    //------------------------------- plot tank level graph ------------------------------------------
      ...
      if ($level[$i-1] == 50){
         ...
         $y2 = intval($tank_y1 + 200 - $old_offset - ($fact73 * $level[$i]));

            imageline( $my_img->img, $x1, $y1, $x2-1, $y1, $blue );
            
            imageline( $my_img->img, $x2-1, $y1, $x2, $y2, $blue );
   ...
   if ($x2 < ($plot_x1 + 864)){  // extend final point to right margin. the following screwed up and drew it red until additional tank values were posted, but that's because the float was on for a while after tank hit 100. i changed draw_red_or_black
      ...
      if ($x1 > $plot_x1){
         ...
         if ($final_float_status > 0){ // make it red. draw_red_or_black() sometimes makes the last tiny bit black, so force it here
            ...
            imageline( $my_img->img, $x1, $y1, $plot_x1 + 864, $y2, $blue );
            //draw_red_or_black($x1, $y1, $plot_x1 + 864, $y2);
         }
      }
   }
...
if ($data_is_old == 1){
   ...
   imagettftext($my_img->img, 32, 0, $big_num_x   , ($tank_y1 + $tank_y2)/2 + 50,  $red, $lib_font_reg, $s);   // img, size, angle, x, y, color, font, text

} else {

   $s = sprintf("%d",$val);

   imagettftext($my_img->img, 32, 0, $big_num_x , ($tank_y1 + $tank_y2)/2 + 50,  $blue, $lib_font_reg, $s);   // img, size, angle, x, y, color, font, text
}
